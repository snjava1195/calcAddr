...
const contract = new ever.Contract(AirdropFactoryAbi, expectedAddress);
 ...
 //Generisan pair   
    const pair = nt.ed25519_generateKeyPair();
 //Napravljen novi keystore
    const keystore = new SimpleKeystore({["0"]: pair});
 //Izbacuje i keypair i kljuc izracunat od njega
    console.log(pair);
    console.log(keystore.getSigner("0"));
    const signer = await keystore.getSigner("0");
    console.log(signer);
    
    //ovde kaze keystore not found
