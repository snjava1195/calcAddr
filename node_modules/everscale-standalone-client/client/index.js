...
const sendUnsignedExternalMessage = async (ctx, req) => {
    ...
    let signedMessage;
    try {
        signedMessage = nekoton.createExternalMessageWithoutSignature(clock, repackedRecipient, payload.abi, payload.method, stateInit, payload.params, 60);
    }
    catch (e) {
        throw invalidRequest(req, e.toString());
    }
...
const sendExternalMessage = async (ctx, req) => {
    ...
    let unsignedMessage;
    try {
        unsignedMessage = nekoton.createExternalMessage(clock, repackedRecipient, payload.abi, payload.method, stateInit, payload.params, publicKey, 60);
    }
    catch (e) {
        throw invalidRequest(req, e.toString());
    }
